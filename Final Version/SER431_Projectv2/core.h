...
GLboolean fuelPodFlag = false;
GLboolean fuelPodLeft = false;
GLboolean fuelPodRight = false;
GLboolean poisionFlag = false;
GLboolean laserShooting = false;
GLboolean startGame = true;
GLboolean gameOver = false;
GLboolean gameMode = false;
GLboolean  viewMode = false;

GLboolean light = true;
GLboolean fog = true;
GLboolean noise = true;
GLboolean terrain = true;
GLboolean bbox = true;
GLboolean sbox = true;
GLboolean reflection = true;
GLboolean shadow = true;
GLboolean particles = true;
GLboolean riverCurve = true;
GLboolean menu = false;
GLboolean fractal = true;
GLboolean lightFlag = true;
GLboolean Materials = true;
...
int height = 500;
float ratio = 1.0;
GLuint mountainDisplay, skyboxDisplay, riverDisplay, f16Display, waterFallDisplay, lavaDisplay, fuelCellDisplay, carDisplay;
GLuint enemyPlaneDisplay, sandDisplay, riverLavaDisplay, lavaFallDisplay;
// meshes
Mesh* mountainMesh;
Mesh* skybox;
Mesh* riverBedMesh;
...
void codedTexture(UINT textureArray[], int n, int type) {
	...
	for (int i = 0; i < TexHeight; i++)
		for (int j = 0; j < TexWidth; j++) {
			...
			else if (type == 2)pixelColor = softLava(t_scale(noise.perlinLava(i * 5, j * 5)));//lavaplanex
			else pixelColor = lavaFire(t_scale(noise.perlinLava(i * 5, j * 5)));
...
// font strokes
void renderStrokeString(float x, float y, float z, float scale, void *font,const char *string) {
	const char *c;
	glPushMatrix();
