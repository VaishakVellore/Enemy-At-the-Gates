...
Vec3f marbleMountain(float a) {
	Vec3f white = Vec3f(1, 1, 1);
	Vec3f color = Vec3f(1, 0, 0);
	return ((1 - a) * white + a * color);
}
// lava
Vec3f lavaMap(float a) {
	Vec3f yellow = Vec3f(0, 0, 3);
	Vec3f red = Vec3f(0,3,3);
	return ( a * red + (1-a )* yellow);
}
...
void codedTexture(UINT textureArray[], int n, int type) {
	...
	for (int i = 0; i < TexHeight; i++)
		for (int j = 0; j < TexWidth; j++) {
			...
			else if (type == 2) pixelColor = lavaMap(t_scale(noise.perlinlava(i * 5, j * 5)));
			else pixelColor = lavaMap(t_scale(noise.perlinlava(i * 5, j * 5)));
