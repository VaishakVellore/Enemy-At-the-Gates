...
Mesh* createPlane(int arena_width, int arena_depth, int arena_cell) {
	...
	for (int i = 0; i < (n*m) - m; i++) {
		...
		for (int t = 0; t < 6; t++) {
			...
			me->face_index_texture.push_back(2);//			me->face_index_texture.push_back(1);//1
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			if(i <=1 || j<=1 || i>= n-2 || j>= m-2)
				me->dot_vertex.push_back(Vec3<GLfloat>(i*arena_cell, 11.5, j*arena_cell));
			else
				me->dot_vertex.push_back(Vec3<GLfloat>(i*arena_cell, rand() % 200, j*arena_cell));
		}
	}
	//texture
	me->dot_texture.push_back(Vec2<GLfloat>(0.0, 0.0));
