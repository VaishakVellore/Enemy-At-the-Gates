...
Mesh* createTerrain(int arena_width, int arena_depth, int arena_cell) {
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			me->dot_vertex.push_back(Vec3<GLfloat>(i*arena_cell, j*arena_cell, 1.0));//100*y_scale(noise.perlinMarble(i,j))
...
Mesh* createPlane(int arena_width, int arena_depth, int arena_cell) {
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++) {
			me->dot_vertex.push_back(Vec3<GLfloat>(i*arena_cell, 0.0, j*arena_cell));//100*y_scale(noise.perlinMarble(i,j))
