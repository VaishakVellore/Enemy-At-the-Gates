...
public:
	...
	double perlinLava(float x, float y) {
		double total = 0;
		double scale = 0.005;
		double p = 2.0; //0.1; //persistence 
		double n = 7.0; //3.0; // number of octaves
	...
	/*double perlinLava(float x, float y) {
		...
		double p = 2.0; //0.1; //persistence 
		double n = 7.0; //3.0; // number of octaves
		double frequency, amplitude, altitude;
		for (int i = 0; i < n; i++) {
			frequency = pow(2, (double)i);
			amplitude = pow(p, (double)-i) * 2;
			altitude = pow(p, (double)i);
			total = total + noise(scale*frequency* x, scale*frequency* y, scale*frequency) * amplitude;
		}
		return total;
	}*/
